...
def _parse_montage_type(recording_folder):
    ...
    number = str(int(prefix)) if prefix.isdigit() else "0" #checks if the prefix is a digitIf not, it defaults to "0".
...
def index_sessions(dataset_path):
    """
    Expects the standard TUSZ layout: .../<split>/<patient_id>/<session_id>/<recording_folder>/*.edf|*.csv_bi
 
    Returns:
        dict: Mapping of session_id -> {
            "split": str,
            "patient_id": str,
            "session_id": str,
            "montage_type": str,
            "edf_paths": list[str],
            "csv_bi_paths": list[str],
        }
    """
    ...
    for root, _dirs, files in os.walk(dataset_path):
        ...
        if not edf_files and not csv_bi_files:
            continue
 
        split = get_split(root)
        parts = os.path.normpath(root).split(os.sep) #clean, split path into its folder components to verify if certain file is in the correct directory structure.
        ...
        session_key = f"{split_prefix}_{patient_id}_{session_id}_{montage_type}" #compiles all components into a unique session key for indexing
 
        edf_paths = [os.path.join(root, f) for f in edf_files] 
        csv_bi_paths = [os.path.join(root, f) for f in csv_bi_files] #Turns filenames into full paths so they can be loaded later
